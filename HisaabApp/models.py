...
class RequestDetail(models.Model):
    ...
    approve_or_rejected_by = models.ForeignKey(NgUser, limit_choice_to={'is_admin':True})
...
class MoneyTransferRequest(models.Model):
    ...
    transfer_to = models.ForeignKey(NgUser,limit_choice_to={'is_admin':False})
...
class AddExpense(models.Model):
    ...
    created_date = models.DateField(auto_now_add=True)
    expense_by = models.ForeignKey(NgUser, limit_choice_to={'is_admin':False})
    category = models.CharField(max_length=30,choice = CATEGORY)
    expense_photo = models.ImageField(upload_to='expenses/%Y/%m/%d')

class RecordPayment(models.Model):
    amount = models.CharField(max_length=5)
    paid_to = models.ForeignKey(NgUser,limit_choice_to={'is_admin':False},null=True)
    facility = models.ForeignKey(Facility)
    paid_by = models.ForeignKey(NgUser,limit_choice_to={'is_admin':True})
    created_date = models.DateField(auto_now_add=True)
